...
`Assets/Scenes/Character Test.unity`

The task MUST NOT be reported as implemented if the three scripts exist but Character Test.unity has not changed.
...
2. Read `Assets/Scenes/Character Test.unity`.
...
10. Save the resulting `Character Test.unity`.
...
12. The Implementer result MUST list `Assets/Scenes/Character Test.unity` as an applied artifact.
...
If Character Test.unity is not actually modified, the Implementer must report failure instead of claiming completion.

## EXACT SCENE INTEGRATION

Use only `Assets/Scenes/Character Test.unity`.
